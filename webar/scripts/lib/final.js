(function(d, a) {
  ...
  var hardenProperty = function() {
    function hardenProperty(reference, properties) {
      for (var i = 0; i < properties.length; i++) {
        var property = properties[i];
        property.enumerable = property.numerable || false;
        property.configurable = true;
        if ('value' in property) property.writable = true;
        Object.defineProperty(reference, property.key, property);
      }
    }
    return function(reference, properties, d) {
      if (properties) hardenProperty(reference['prototype'], properties);
      if (d) hardenProperty(reference, d);
      return b;
    };
  }();
  u;
  ...
  var A = function(c, d) {
      ...
      if (b['type'] === 'offer') {
        ...
        d['forEach'](function(d, h) {
          ...
          if (!o && !b['isDatachannel']) {
            ...
            var q = m(l, n);
            c['_transceive'](b, q['codecs']['length'] > 0x0, false);
          }
        });
      }
    ...
    b['prototype']['setRemoteDescription'] = function(f) {
      ...
      l['forEach'](function(l, k) {
        ...
        if (f['type'] === 'offer' && !t) {
          ...
          b['_transceive'](b['transceivers'][k], false, x);
    ...
    b['prototype']['_maybeFireNegotiationNeeded'] = function() {
      ...
      c['setTimeout'](function() {
        if (a['needNegotiation'] === false) {
          return;
        }
        a['needNegotiation'] = false;
    ...
    b['prototype']['createOffer'] = function() {
      ...
      if (b) {
        ...
        if (b['offerToReceiveAudio'] !== undefined) {
          if (b['offerToReceiveAudio'] === true) {
            e = 0x1;
          } else if (b['offerToReceiveAudio'] === false) {
            e = 0x0;
          } else {
        ...
        if (b['offerToReceiveVideo'] !== undefined) {
          if (b['offerToReceiveVideo'] === true) {
            f = 0x1;
          } else if (b['offerToReceiveVideo'] === false) {
            f = 0x0;
          } else {
  ...
  var CameraVideoSource = function() {
    function a(b) {
      g(this, a);
      this['_constraints'] = b;
    }
  ...
  function getCameraParams(width, height) {
    return {
      'cameraFOV': 40,//拍摄距离  视野角值越大，场景中的物体越小
      'cameraAspect': width / height, //纵横比
      'cameraNear': 0.1,//相机离视体积最近的距离
      'cameraFar': 1000,//相机离视体积最远的距离
      'cameraPosition': new THREE.Vector3(100, 0, -100)
    };
  };

  var Renderer = function() {
    function Renderer(c, CameraParams) {
      g(this, Renderer);
      this._videoDevice = c;
      var width = c.width;
      var height = 599;//c['height'];
      CameraParams = Object['assign']({}, getCameraParams(width, height), CameraParams);
      this._scene = new THREE.Scene();
      this._camera = new THREE.PerspectiveCamera(CameraParams.cameraFOV, CameraParams.cameraAspect, CameraParams.cameraNear, CameraParams.cameraFar);
      // every object is initially created at ( 0, 0, 0 )
      // we'll move the camera back a bit so that we can view the scene
      this._camera.position.set(CameraParams.cameraPosition);
      ...
      this._containerElm.appendChild(this._videoDevice.videoElement);
      this._videoDevice.videoElement.style.position = 'absolute';
      this._containerElm.appendChild(this._renderer.domElement);
      this._renderer.domElement.style.position = 'absolute';
    }
    hardenProperty(Renderer, [{
      'key': 'start',
      'value': function toAnimate(updateFrame) {
        var render = this;
        function animate() {
          ...
          updateFrame();
          render._renderer.clear();
          render._renderer.render(render._scene, render._camera);
        };
        animate();
      }
    }, {
      'key': 'domElement',
      'get': function getDomElement() {
        return this._containerElm;
      }
    }, {
      'key': 'scene',
      'get': function getScene() {
        return this._scene;
      }
    }]);
    return Renderer;
