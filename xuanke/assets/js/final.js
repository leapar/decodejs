    ...
    window.Model = EventEmitter.extend(function (a) {
      ...
      rotate: function (a, b) {
        ...
        if (360 == this.meta.range.v[1]) {
          ...
          this.angleV = Math.min(this.angleV, /*this.meta.range.v[1]*/Math.ceil(180/24*23));
          //this.angleV %= 180
      ...
      buildImageURL: function (a) {
        ...
        var b = "./imgs/{2}_{3}.jpg".format(this.name, a.sizeLevel || this.sizeLevel, Math.round((a.angleV || this.angleV) / 180 * 24) , parseInt((a.angleH || this.angleH) / 360 * 25), this.meta.timestamp);
        return b
      },
      load: function () {
        var a = this;
      ...
      _loadMeta: function (a) {
        var b = "./models/meta.json?name={0}&t={1}".format(a, (new Date).getTime());
        return $.ajax({
          url: b,
          type: "GET",
          dataType: "json",
    ...
    window.MaskController = function (q) {
      ...
      function t(g) {
        ...
        switch (f) {
          case "h":
            if (Math.abs(d) > 10) {
              d = n.adjustDeltaX(g.position[1], d);
              d = Math.round(d / 10) * 5;
              y[0] = 0;
              y[1] = 0;
              m.rotate(0, d);
      ...
      var u = klass(function (a, b) {
        ...
        adjustDeltaX: function (a, b) {
          if (this._referenceBoxies.isLine()) {
            return this._referenceBoxies.isReversed() ? -b : b
          } else {
            if(this._referenceBoxies.isTopVisible() && a < this._referenceBoxies.topArea().center[1] ) {
              b = -b
            } else {
              this._referenceBoxies.isBottomVisible() && a > this._referenceBoxies.bottomArea().center[1] && (b = -b)
            } 
            return b
        
          }
         // return this._referenceBoxies.isLine() ?  : (this._referenceBoxies.isTopVisible() && a < this._referenceBoxies.topArea().center[1] ? b = -b : this._referenceBoxies.isBottomVisible() && a > this._referenceBoxies.bottomArea().center[1] && (b = -b), b)
        },
