    ...
    window.Model = EventEmitter.extend(function(a) {
      ...
      rotate: function(a, b) {
        //this.angleV += Math.round(a), this.angleH += Math.round(b), this.angleH += 360, this.angleH %= 360, 360 == this.meta.range.v[1] ? (this.angleV += 360, this.angleV %= 360) : (this.angleV = Math.min(this.angleV, this.meta.range.v[1]), this.angleV = Math.max(this.angleV, this.meta.range.v[0])), this.emit("imageChanged", "rotate")
           // console.log(a,b);
           //this.angleV += Math.round(a);
            this.angleH += Math.round(b);
            this.angleH += 360;
            this.angleH %= 360;
   
            //this.angleV *= dTemp;
           // console.log(this.angleH);
            if(this.angleV + Math.round(a) >= 180) {
                
            } else if(this.angleV + Math.round(a) < 0) {
                
            } else {
                this.angleV += Math.round(a);
                if(360 == this.meta.range.v[1]) {
                    this.angleV += 360;
                    this.angleV %= 360;
                } else {
                    
                    //this.angleV = parseInt(this.angleV / this.meta.range.v[1]);
                    //this.angleV = Math.min(this.angleV , this.meta.range.v[1] * 360);
                   // this.angleV = Math.max(this.angleV, this.meta.range.v[0])
                    this.angleV += 180;
                    this.angleV %= 180;
                }
            }
            ...
            /*if(360 == this.meta.range.h[1]) {
                this.angleH += 360;
                this.angleH %= 360;
            } else {
                this.angleH = Math.min(this.angleH, this.meta.range.h[1]);
                this.angleH = Math.max(this.angleH, this.meta.range.h[0])
            }*/
           /* var dTemp = 360 / this.meta.range.h[1];
            this.angleH /= dTemp;
            this.angleH = parseInt(this.angleH);*/
           // console.log(this.angleH);
            this.emit("imageChanged", "rotate")
      },
      move: function(a, b) {
      ...
      buildImageURL: function(a) {
        ...
        var angleV = parseInt(a.angleV / 180 * (this.meta.range.v[1] + 1));
        var angleH = parseInt(a.angleH / 360 * (this.meta.range.h[1] + 1))

        var b = "./imgs/{2}_{3}.jpg".format(this.name, 
            a.sizeLevel || this.sizeLevel, 
            angleV/* || this.angleV*/, 
            angleH/* || this.angleH*/, 
            this.meta.timestamp);
          //var b = "./imgs/{2}_{3}.jpg".format( a.angleV || this.angleV, a.angleH || this.angleH);
        //console.log(b);  
        return b
      },
      load: function() {
      ...
      _loadMeta: function(a) {
            var b = "./models/meta.json?name={0}&t={1}".format(a, /*(new Date).getTime()*/1517635540139);//"/xkmodels/meta.json?name={0}&t={1}["format"] getTime"
        return $.ajax({
          url: b,
          type: "GET",
          dataType: "json",
  ...
  function() {
    ...
    window.MaskController = function(q) {
      ...
      function t(g) {
        ...
        switch (f) {
          case "h":
            Math.abs(d) > c ? (d = n.adjustDeltaX(g.position[1], d)* -1, d = Math.round(d / c) * a, y[0] = 0, y[1] = 0, m.rotate(0, d)) : y[0] = d;
            break;
          case "v":
